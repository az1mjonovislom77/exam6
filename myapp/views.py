...
def export_data(request):
    ...
    if format == 'csv':
        ...
        data = list(Customer.objects.all().values('id','name', 'email', 'phone', 'billing_address','joined_date'))
        ...
        customers = Customer.objects.all().values('id', 'name', 'email', 'phone', 'billing_address', 'joined_date')
        df = pd.DataFrame(customers)
        if 'joined_date' in df.columns and df['joined_date'].dtype == 'datetime64[ns, UTC]':
            df['joined_date'] = df['joined_date'].dt.tz_localize(None)
        if 'joined_date' in df.columns:
            df['joined_date'] = df['joined_date'].dt.strftime('%Y-%m-%d %H:%M:%S')
